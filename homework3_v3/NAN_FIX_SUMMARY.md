...
3. **FP16 Numerical Instability**: FP16 (float16) precision can cause overflow/underflow:
   - FP16 has limited range (±65,504)
   - Large logits can overflow to `Inf`
   - Cross-entropy loss with `Inf` logits → `NaN` loss
   - This is especially problematic in the grader's `compute_loss` function

4. **np.clip Doesn't Handle NaN**: The grader's `normalize_score` function uses `np.clip`, which doesn't clip NaN values - it just returns NaN, which then propagates to the final score calculation.
...
### 1. FP16 → FP32 Default (`base_llm.py`)

**Changed default inference precision:**
- **Before**: Models loaded in FP16 by default (faster but unstable)
- **After**: Models loaded in FP32 by default (slower but stable)
- **Why**: FP16 can cause logits to overflow to `Inf`, leading to `NaN` in cross-entropy loss
- **Impact**: Prevents FP16 overflow issues in the grader's `compute_loss` function
- **Note**: Can still enable FP16 via `USE_FP16_INFERENCE=1` environment variable (not recommended for grading)

### 2. Enhanced Generation Validation (`base_llm.py`)
...
### 4. Defensive Accuracy Checks (`data.py`)
...
1. `homework3_v3/homework/base_llm.py`
   - **Changed default to FP32** for inference (prevents FP16 overflow → NaN)
   - Enhanced `generate()` method with token count validation
   - Enhanced `batched_generate()` method with token count validation
   - Added `use_fp32_for_inference` parameter for explicit control
...
- ✅ Models load in FP32 by default (prevents FP16 overflow → NaN)
...
- ✅ Logits are always finite (no FP16 overflow to Inf)
...
1. **Use Stable Precision**: Default to FP32 for inference (prevents FP16 overflow)
2. **Prevent NaNs at Source**: Ensure generations always produce valid, tokenizable outputs
3. **Validate Before Passing**: Check all values passed to the grader are finite
4. **Defensive Defaults**: Use 0.0 instead of NaN when values are invalid
...
## FP16 Overflow Details

### Why FP16 Causes NaN

1. **Limited Range**: FP16 can only represent values from -65,504 to +65,504
2. **Logit Overflow**: Large logits (common in language models) can exceed this range
3. **Inf Propagation**: Overflowed logits become `Inf`
4. **NaN in Loss**: `cross_entropy(logits=Inf, labels)` → `NaN` loss
5. **Grader Crash**: `NaN` propagates through `normalize_score` → `ValueError`
...
| Stability | Can overflow → NaN | Stable, no overflow |
| Range | ±65,504 | ±3.4×10³⁸ |

**Recommendation**: Use FP32 for grading (stability > speed). Use FP16 only if you need speed and can handle potential NaN issues.
