...
DEFAULT_LORA_RANK = 8  # Increased from 4 to improve model capacity
...
def train_model(
    ...
    **_: Any,
    ...
    # Create LoRA config with increased rank for better capacity
    config = LoraConfig(
        task_type="CAUSAL_LM",
        target_modules="all-linear",
        bias="none",
        r=DEFAULT_LORA_RANK,
        lora_alpha=DEFAULT_LORA_RANK * 4,  # Keep alpha = 4 * rank for stability
        lora_dropout=0.0,
    )
    ...
    # Training arguments with improved stability and accuracy
    training_args_dict = {
