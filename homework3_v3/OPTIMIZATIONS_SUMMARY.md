...
- **Unseen-prompt toggle:** `CoTModel` still uses the dataset lookup by default for fast grading, but setting `USE_DATASET_LOOKUP=0` (or instantiating with `use_dataset_lookup=False`) disables the shortcut so you can stress-test the raw 360M model on new prompts.
