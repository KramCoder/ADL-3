...
def tokenize(tokenizer, question: str, answer: str):
    """Tokenize question and answer, masking question tokens in labels."""
...
class TokenizedDataset:
    ...
    def __getitem__(self, idx):
        if idx >= len(self.data):
            raise IndexError(f"Index {idx} out of range for dataset of length {len(self.data)}")
        try:
            data_item = self.data[idx]
            formatted_data = self.format_fn(*data_item)
            return tokenize(self.tokenizer, **formatted_data)
        except (IndexError, TypeError, ValueError) as e:
            raise ValueError(f"Error processing data item at index {idx}: {e}. Data item: {data_item if 'data_item' in locals() else 'N/A'}")
...
def train_model(
    ...
    **_: Any,
    ...
    if not rft_data or len(rft_data) == 0:
        raise ValueError(f"RFT dataset is empty or invalid. Expected non-empty list, got: {type(rft_data)} with length {len(rft_data) if isinstance(rft_data, list) else 'N/A'}")
    
    if not isinstance(rft_data, list):
        raise ValueError(f"RFT dataset must be a list, got: {type(rft_data)}")
    
    if len(rft_data[0]) < 3:
        raise ValueError(f"RFT data format incorrect. Expected [question, correct_answer, reasoning], got item with {len(rft_data[0])} elements")
